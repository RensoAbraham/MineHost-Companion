...
async fn main() {
    ...
    let app = Router::new()
        ...
        .route("/install", get(install_server))
        .with_state(shared_state);

    let addr = SocketAddr::from(([127, 0, 0, 1], 8000));
    println!("Servidor backend (con /install) escuchando en http://{}", addr);
...
async fn install_server(
    ...
    match downloader::download_paper_server(&app_state.http_client, "1.20.1", save_path).await {
        Ok(hash) => {
            Json(InstallResponse {
                message: "install_success".to_string(),
                hash: Some(hash),
            })
        }
        Err(e) => {
            println!("[Error /install] {}", e.to_string());
            Json(InstallResponse {
